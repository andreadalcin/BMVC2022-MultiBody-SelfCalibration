...
def norm7Point(X, Y):
    ...
    M = np.zeros((N, 9))
    for i in range(N):
        M[i, :] = [X[0, i] * Y[0, i], X[1, i] * Y[0, i], Y[0, i], X[0, i] * Y[1, i], X[1, i] * Y[1, i], Y[1, i],
                   X[0, i], X[1, i], 1]
    ...
    b = (
                f11 * g22 * g33 - f11 * g23 * g32 - f12 * g21 * g33 + f12 * g23 * g31 + f13 * g21 * g32 - f13 * g22 * g31 - f21 * g12 * g33 + f21 * g13 * g32 + f22 * g11 * g33 - f22 * g13 * g31
                - f23 * g11 * g32 + f23 * g12 * g31 + f31 * g12 * g23 - f31 * g13 * g22 - f32 * g11 * g23 + f32 * g13 * g21 + f33 * g11 * g22 - f33 * g12 * g21)
    c = (
                f11 * f22 * g33 - f11 * f23 * g32 - f11 * f32 * g23 + f11 * f33 * g22 - f12 * f21 * g33 + f12 * f23 * g31 + f12 * f31 * g23 - f12 * f33 * g21 + f13 * f21 * g32 - f13 * f22 * g31
...
def sampsonDistanceH(X, Y, H):
    ...
    G1 = np.vstack((H[0, 0] - Y[0, :] * H[2, 0], H[0, 1] - Y[0, :] * H[2, 1],
                    -X[0, :] * H[2, 0] - X[1, :] * H[2, 1] - H[2, 2], np.zeros((1, N))))
    G2 = np.vstack((H[1, 0] - Y[1, :] * H[2, 0], H[1, 1] - Y[1, :] * H[2, 1], np.zeros((1, N)),
                    -X[0, :] * H[2, 0] - X[1, :] * H[2, 1] - H[2, 2]))
